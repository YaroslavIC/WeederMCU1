...

{
 ...
 void WheelData::ReadAS5600_Curr(float curr_) {
 		...
 		if (ws==wsLeft) {
 			if (Target_Speed >=0) {
                if (tmpangle>=angle[MAX_ANGLE_WHEEL_ARRAY - 1]) {
             	 ...
             	 delta_angle = (360 - angle[MAX_ANGLE_WHEEL_ARRAY - 1]) + tmpangle;
                }
 			} else {
               if (tmpangle<=angle[MAX_ANGLE_WHEEL_ARRAY - 1]) {
            	 delta_angle = angle[MAX_ANGLE_WHEEL_ARRAY - 1] - tmpangle;
 		...
 		speed[MAX_ANGLE_WHEEL_ARRAY - 1] =  Current_Speed;
 		Disired_Target_diff[MAX_ANGLE_WHEEL_ARRAY - 1] = Target_Speed - Current_Speed;
...
void WheelData::Calculation(void)
{
	...
	if (fabsf(Target_Speed) > 0) {
		...
		PID_value_P = PID_P * (Target_Speed - speed[MAX_ANGLE_WHEEL_ARRAY-1]);


		PID_Sum_I = PID_Sum_I + (Target_Speed - speed[MAX_ANGLE_WHEEL_ARRAY-1])*0.25;
		PID_value_I = PID_I * PID_Sum_I;


 	    Derror = ((Disired_Target_diff[MAX_ANGLE_WHEEL_ARRAY] - Disired_Target_diff[MAX_ANGLE_WHEEL_ARRAY - 1])) / 0.25;
